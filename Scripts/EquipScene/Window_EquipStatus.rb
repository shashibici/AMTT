...
class Window_EquipStatus1 < Window_Base
  ...
  def refresh
    ...
    self.contents.blt(0, @edge, bitmap, rect)
    bitmap.dispose
    
    # 描绘名字(72+108=180)
    color = Color.new(255, 255, 255)
    draw_a_line(color, @face_size + RW, @edge, 108, WLH, @name, 1)
    
    # 描绘等级，名字右边(180+40=220,220+50=270)
    color = system_color
    draw_a_line(color, @face_size + 108 + RW , @edge, 40, WLH, "等级", 1)
    color = Color.new(255,255,255)
    draw_a_line(color, @face_size + 108 + 40 + RW, @edge, 40, WLH, @lvl, 1)
    
    # 描绘主属性（270+40=310）
    color = system_color
    draw_a_line(color, @face_size + 108 + 80 + RW , @edge, 40, WLH, "属性", 2)    
    
    case @type
    when 1
      bitmap = Cache.face("力量主属性")
    when 2
      bitmap = Cache.face("敏捷主属性")
    when 3
      bitmap = Cache.face("智力主属性")
    end
    rect.x = 0
    rect.y = 0
    rect.width = 32
    rect.height = 32    
    self.contents.blt(@face_size + 108 + 135 + RW, @edge, bitmap, rect)
    bitmap.dispose
    
    #-- 经验------------------------------------------------------ 
    # 描绘经验槽-------------------------------------
    preexp = @actor.exp_list[@lvl] > 0 ? @actor.exp_list[@lvl] : 0
    nextexp = @actor.exp_list[@lvl+1] > 0 ? @actor.exp_list[@lvl+1] : 0
    diffexp = nextexp - preexp
    current = @exp - preexp
    rate = 1.0 * current / diffexp
    
    
    gw = Integer(260 * rate)
    if rate > 0.5
      gc1 = Color.new(0,255,Integer(255*(1-rate*2)))
    else
      gc1 = Color.new(0,Integer(255*rate*2),255)
    end
    gc2 = getColor("light red")
    
    # 填满指定的颜色
    self.contents.fill_rect(@face_size+RW, WLH*2+@edge*8, 260, 6, gc2)
    self.contents.fill_rect(@face_size+RW, WLH*2+@edge*8, gw, 6, gc1)
    
    # 绘制文字
    self.contents.font.color = system_color
    self.contents.draw_text(@face_size+RW, WLH+@edge*4, 30, WLH, "经验")
    xr = 330
    # 计算经验的长度
    if current > 9 and current <= 99
      bk1 = 5
    elsif current > 99 and current <= 999
      bk1 = 6
    elsif current > 999 and current <= 9999
      bk1 = 7
    elsif current > 9999 and current <= 99999
      bk1 = 8
    elsif current > 99999 and current <= 999999
      bk1 = 9
    elsif current > 999999 and current <= 9999999
      bk1 = 10
    elsif current > 9999999 
      bk1 = 11
    else 
      bk1 = 4
    end  
    # 
    if diffexp > 9 and diffexp <= 99
      bk2 = 5
    elsif diffexp > 99 and diffexp <= 999
      bk2 = 6
    elsif diffexp > 999 and diffexp <= 9999
      bk2 = 7
    elsif diffexp > 9999 and diffexp <= 99999
      bk2 = 8
    elsif diffexp > 99999 and diffexp <= 999999
      bk2 = 9
    elsif diffexp > 999999 and diffexp <= 9999999
      bk2 = 10
    elsif diffexp > 9999999
      bk2 = 11
    else 
      bak2 = 4
    end 
    ...
    self.contents.draw_text(xr - 10*(bk1+bk2+1), WLH+@edge*4, bk1*10, WLH, current, 2)
    self.contents.draw_text(xr - 10*(bk2+1),     WLH+@edge*4, 10, WLH, "/", 2)
    self.contents.draw_text(xr - 10*bk2,         WLH+@edge*4, bk2*10, WLH, diffexp, 2)
  ...
  def need_refresh?
    ...
    return true if @type != @acotr.type
    return false
  end  
