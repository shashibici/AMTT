...
class Window_Consult_Status1 < Window_Base
	...
	def refresh
		...
		#-------------描绘角色--------------------------
		# 描绘脸部 (64*64)
		dst_rect = Rect.new((@pic_size-@face_size)/2,(@pic_size-@face_size)/2,@face_size,@face_size)
		bitmap = FrameFactory.getBitmap("Graphics/battlers/", @actor_monstor_face)
		rect = Rect.new(0, 0, @face_size, @face_size) 
...
class Window_Consult_Status3 < Window_Base
	...
	# ● 初始化对象
	#--------------------------------------------------------------------------
	def initialize(x,y,actor, w = 416+32, h = 84+32)
		super(x, y, w, h)
		self.opacity = 0
		self.contents.font.size = 20
		self.contents.font.name = "黑体" 
	...
	def refresh
		...
		draw_a_line(color, 0, @edge+WLH*2, 80, WLH, "经验: ", 0)
		draw_a_line(color, 80, @edge+WLH*2, self.contents.width/2-80, WLH, @exp.ceil, 0)
	end
	#--------------------------------------------------------------------------
	# ● 是否需要重绘?
	#--------------------------------------------------------------------------
